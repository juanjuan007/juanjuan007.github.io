...
> 兩者都先照右端點排序。
> 
> 找到第一個不相同的線段 $S_{i}$ , $S_i^{\prime}$，
>
> 因為前者的右端點較左，因此把 $S_i^{\prime}$ 替換成 $S_{i}$ 不會影響其它段，同時也會是最佳解之一。
>
...
假設最終長度是 $x$ ， 那總代價會是 $\sum_{i = 1}^{n} abs(x - p_{i})$。
...
考慮每個位置是右端點的狀況，可以發現左端點是單調遞增，維護的區間內至多 $k$ 種。
